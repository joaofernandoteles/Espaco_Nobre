                            ...
                            <form id="formCadastro">
                                <div class="row">
                                    <div class="col-12 col-md-12 form-group">
                                        ...
                                        <input type="file" class="form-control" id="Imagem" name="Imagem" accept="image/*" maxlength="255" <?= isset($_GET['IDBannerHome']) ? '' : 'required' ?>>
                                    </div>
                                </div>
                                <div class="col-md-12">
                                    ...
                                    <input type="hidden" name="IDBannerHome" id="IDBannerHome">
                                </div>
                            </form>
                        </div>
                    </div>
    ...
    <script>
        $('#formCadastro').validate({
            ...
            submitHandler: function(form) {
                ...
                var option = $('#IDBannerHome').val() == '' ? 'insert' : 'update';

                $.ajax({
                    type: 'POST',
                    url: 'assets/ajax/banners.php?option=' + option,
                    data: formData,
                    processData: false,
                    contentType: false
        ...
        <?php if (isset($_GET['IDBannerHome']) && !empty($_GET['IDBannerHome'])) { ?>

            $.post('./assets/ajax/banners.php?option=select', {
                IDBannerHome: '<?= $_GET['IDBannerHome'] ?>'
                })
                .done(function(response) {
                    response = JSON.parse(response);

                    $('#IDBannerHome').val(response.IDBannerHome);
                });
